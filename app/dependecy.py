...
from app.repository import TaskRepository, TaskCache, UserRepository, CatRepository
...
async def get_cats_repository(db_session: AsyncSession = Depends(get_db_session)) -> CatRepository:
    return CatRepository(db_session)


async def get_cat_service(cat_repository: CatRepository = Depends(get_cats_repository)) -> CategoryService:
    return CategoryService(cat_repository=cat_repository)
